...
@interface USDeleBindCardMsgCodeViewController()<UIPickerViewDelegate>
...
@end
@implementation USDeleBindCardMsgCodeViewController
-(void)viewDidLoad{
    [super viewDidLoad];
    self.title = @"验证码";
...
-(void)createViews{
    ...
    [bindBtton addTarget:self action:@selector(sureCancleBind) forControlEvents:UIControlEventTouchUpInside];
...
-(void)sureCancleBind{
    ...
    HYLog(@"%@",params) ;
    [USWebTool POSTWIthTip:@"bindbankcardcilent/canclesBindCard.action" showMsg:@"正在解除绑定.." paramDic:params success:^(NSDictionary * dic)  {
...
-(void)getValidcodeClick:(UIButton *)button{
    [USWebTool POSTWIthTip:@"huiFuPayClientController/sendMsgCode.action" showMsg:@"正在获取验证码..." paramDic:@{@"user_mobile":_card_mobile,@"business_type":@"105"} success:^(NSDictionary * dic)  {
        //设置短信订单日期和订单号

...
-(void)createRebackDetailView{
    UITextView *tipText =[[UITextView alloc]initWithFrame:CGRectMake(10, 10, kAppWidth-20, 100)];
    tipText.backgroundColor = [UIColor clearColor];
    tipText.editable = NO;
    tipText.font = [UIFont systemFontOfSize:kCommonFontSize_15];
    tipText.text = @"友情提示:\n还款没有成功或者刷新的请等待至少5分钟后在刷新还款!\n还款总额=还款金额+利息+服务费+支付手续费";
    tipText.textColor = HYCTColor(175, 175, 175);
    [self.view addSubview:tipText];
    //
    UIView *centerView = [[UIView alloc]initWithFrame:CGRectMake(1, tipText.y+tipText.height+10, kAppWidth-2, 100)];
    ...
    //
    centerView = [[UIView alloc]initWithFrame:CGRectMake(1, centerView.y+centerView.height+20, kAppWidth-2, 60)];
    centerView.layer.borderColor = [HYCTColor(221, 221, 221) CGColor];
    centerView.layer.borderWidth = 0.8;
    [centerView setBackgroundColor:[UIColor whiteColor]];
    tipLB = [USUIViewTool createUILabelWithTitle:@"还款金额:" fontSize:kCommonFontSize_20 color:[UIColor blackColor] heigth:kCommonFontSize_20];
    tipLB.frame = CGRectMake(kMargin, (centerView.height-kCommonFontSize_20)*0.5, 90, kCommonFontSize_20);
    [centerView addSubview:tipLB];
    //
    ...
    //buttons
    _bttonsArray = [NSMutableArray arrayWithCapacity:2];
    UIButton *allBt = [USUIViewTool createButtonWith:@"全部还款"];
    allBt.backgroundColor = [UIColor orangeColor];
    allBt.frame = CGRectMake(kMargin*3, centerView.y+centerView.height+kMargin*3, (kAppWidth-kMargin*6)*0.5, 40);
    allBt.tag = 0;
    ...
    switch (sender.tag) {
        ...
        default:
        {
            ...
            if (!([_quotaField.text floatValue]>=kBase)) {
                [MBProgressHUD showError:@"输入的金额必须大于等于10元..."];
                return;

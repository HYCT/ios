...
{
    if (strResult.strScanned!=nil&&strResult.strScanned.length>0) {
        if (_actionType==1) {
            USScanCodeResultViewController *scanResultVC = [[USScanCodeResultViewController alloc]init];
            scanResultVC.orderStr = strResult.strScanned;
        ...
        [MBProgressHUD showError:@"二维码图片信息有误,请重试..."];
        [self.navigationController popViewControllerAnimated:YES];
    }
}

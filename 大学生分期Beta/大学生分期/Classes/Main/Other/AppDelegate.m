...
- (void)application:(UIApplication *)application didReceiveRemoteNotification:(NSDictionary *)userInfo fetchCompletionHandler:(void (^)(UIBackgroundFetchResult))completionHandler
{
    @try {
        ...
        HYLog(@"收到消息：%@",userInfo) ;
        //赋值
        _msgtype = userInfo[@"type"] ;
        //更新用户数据
        [self updateSetCustomerData:userInfo[@"data"]] ;
        // 打印到日志 textView 中
        NSLog(@"********** iOS7.0之后 background **********");
        // 应用在前台 或者后台开启状态下，不跳转页面，让用户选择。
        if (application.applicationState == UIApplicationStateActive || application.applicationState == UIApplicationStateBackground) {
            NSLog(@"acitve or background");
            UIAlertView *alertView =[[UIAlertView alloc]initWithTitle:userInfo[@"title"] message:userInfo[@"aps"][@"alert"] delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
            [alertView show];
        }
        else//杀死状态下，直接跳转到跳转页面。
        {
            [self MsgToController:_msgtype] ;
...
//根据收到的消息跳转到不同的视图
-(void)MsgToController:(NSString *)type{
    @try {
        HYLog(@"msg:%@",type) ;
        //退出登录
        if ([@"-1" isEqualToString:type ]) {
            USLoginViewController *skipCtr = [[USLoginViewController alloc]init];
            skipCtr.accountTelephone = _accountTelephone ;
            skipCtr.accountPwd =_accountPwd ;
            skipCtr.hidesBottomBarWhenPushed = YES ;
            // 根视图是nav 用push 方式跳转
            ...
            //我要还款
            USRebackListViewController *controller = [[USRebackListViewController alloc]init];
            controller.hidesBottomBarWhenPushed = YES ;
            [_tabBarCtr.selectedViewController pushViewController:controller animated:YES];
    ...
    @catch (NSException *exception) {
        HYLog(@"MsgToController error:%@",exception) ;
    }
    @finally {
...
//更需customer
-(void)updateSetCustomerData:(NSDictionary *)data{
    //退出登录
    if ([@"-1" isEqualToString:_msgtype ]) {
        [USUserService logout];
        
    }else if ([@"1" isEqualToString:_msgtype] && data != nil) {
        //送优惠券h和提额礼包时，重新设置account
        USAccount *account = [USAccount accountWithDic:data] ;
        [USUserService saveAccount:account] ;
    }else if ([@"2" isEqualToString:_msgtype] && data != nil) {
        //借款消费，重新设置account
        USAccount *account = [USAccount accountWithDic:data] ;
        [USUserService saveAccount:account] ;
    }else if([@"3" isEqualToString:_msgtype] && data != nil){
        //消息推送，重新设置account
        USAccount *account = [USAccount accountWithDic:data] ;
        [USUserService saveAccount:account] ;
    }
